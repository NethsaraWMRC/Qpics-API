...
exports.updateImage = async (req, res) => {
  try {
    ...
    const currentImage = await ImageData.findByIdAndUpdate(imageId, updateImg)
      .then(() => {
      ...
      .catch((err) => {
        ...
        res
          .status(500)
          .send({ status: "error with update record", error: err.message });
      });
  } catch (error) {
  ...
  try {
    ...
    await ImageData.findByIdAndDelete(imageId)
      ...
      .catch((err) => {
        ...
        res
          .status(500)
          .send({ status: "error with delete record", error: err.message });
      });
  ...
  try {
    ...
    const currentImage = await ImageData.findById(imageId)
      ...
      .catch((err) => {
        ...
        res
          .status(500)
          .send({ status: "error with fetch record", error: err.message });
      });
  } catch (error) {
    console.error("Error:", error);
    res.status(500).json({ error: "An error occurred" });
...
exports.updateImageViews = async (req, res) => {
  try {
    ...
    const image = await ImageData.findByIdAndUpdate(
      imageId,
      { $inc: { views: 1 } },
      { new: true }
    );

    await userData.findByIdAndUpdate(image.user, {
      $inc: { views: 1 },
    });

    return res.status(200).json("Views updated successfully!");
  } catch (error) {
